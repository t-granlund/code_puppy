...
def test_summarize_messages_nothing_to_summarize():
    ...
    msg1 = MagicMock()
    msg2 = MagicMock()
    with patch.object(agent, 'split_messages_for_protected_summarization',
...
def test_windows_key_listener_ctrl_x():
    ...
    with patch('msvcrt.kbhit', side_effect=[True, False]), \
...
def test_posix_key_listener_ctrl_x():
    ...
    with patch('sys.stdin', mock_stdin), \
         ...
         patch('tty.setcbreak'), \
         patch('select.select', side_effect=[([mock_stdin], [], []), ([mock_stdin], [], [])]):
...
def test_posix_key_listener_cancel_agent():
    ...
    with patch('sys.stdin', mock_stdin), \
         ...
         patch('code_puppy.agents.base_agent.get_cancel_agent_char_code', return_value='\x04'):
        agent._listen_for_ctrl_x_posix(stop_event, lambda: None, on_cancel_agent=lambda: cancelled.append(True))
