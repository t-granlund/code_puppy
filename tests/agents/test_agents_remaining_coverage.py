...
def test_code_reviewer():
    _test_reviewer_agent('code_puppy.agents.agent_code_reviewer', 'CodeQualityReviewerAgent')

def test_cpp_reviewer():
    _test_reviewer_agent('code_puppy.agents.agent_cpp_reviewer', 'CppReviewerAgent')

def test_golang_reviewer():
    _test_reviewer_agent('code_puppy.agents.agent_golang_reviewer', 'GolangReviewerAgent')

def test_javascript_reviewer():
    _test_reviewer_agent('code_puppy.agents.agent_javascript_reviewer', 'JavaScriptReviewerAgent')

def test_python_reviewer():
    _test_reviewer_agent('code_puppy.agents.agent_python_reviewer', 'PythonReviewerAgent')

def test_typescript_reviewer():
    _test_reviewer_agent('code_puppy.agents.agent_typescript_reviewer', 'TypeScriptReviewerAgent')

def test_security_auditor():
    _test_reviewer_agent('code_puppy.agents.agent_security_auditor', 'SecurityAuditorAgent')
...
def test_creator_agent_get_system_prompt_with_uc_tools():
    ...
    with patch('code_puppy.plugins.universal_constructor.registry.get_registry', return_value=mock_registry):
        prompt = agent.get_system_prompt()
...
def test_creator_validate_agent_json_valid():
    ...
    with patch('code_puppy.agents.agent_creator_agent.get_available_tool_names',
...
def test_creator_get_agent_file_path():
    ...
    from code_puppy.agents.agent_creator_agent import AgentCreatorAgent
    agent = AgentCreatorAgent()
    with patch('code_puppy.agents.agent_creator_agent.get_user_agents_directory',
...
def test_creator_create_agent_json_success(tmp_path):
    ...
    with patch('code_puppy.agents.agent_creator_agent.get_available_tool_names',
               return_value=['list_files']), \
         patch('code_puppy.agents.agent_creator_agent.get_user_agents_directory',
...
def test_json_agent_uc_import_error(tmp_path):
    ...
    with patch.dict('sys.modules', {'code_puppy.plugins.universal_constructor.registry': None}):
        tools = agent.get_available_tools()
