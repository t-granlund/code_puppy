...
def test_load_tasks_json_decode_error(tmp_path):
    ...
    with patch.object(config, 'SCHEDULES_FILE', str(schedules_file)), \
...
def test_update_task_not_found(tmp_path):
    ...
    with patch.object(config, 'SCHEDULES_FILE', str(schedules_file)), \
         patch.object(config, 'ensure_scheduler_dirs', return_value=None):
        task = ScheduledTask(id="nonexistent", name="test", prompt="echo", schedule_value="1h")
...
def test_should_run_task_disabled():
    ...
    task = ScheduledTask(id="t1", name="test", prompt="echo", schedule_value="1h", enabled=False)
...
def test_daemon_loop_runs_tasks():
    ...
    task = ScheduledTask(
        id="t1", name="test", prompt="echo hi", schedule_value="1m",
        enabled=True, last_run=(datetime.now() - timedelta(hours=1)).isoformat()
    )
    ...
    with patch.object(daemon, 'load_tasks', return_value=[task]), \
...
def test_daemon_loop_task_failure():
    ...
    task = ScheduledTask(
        id="t2", name="fail-test", prompt="false", schedule_value="1m",
        enabled=True, last_run=(datetime.now() - timedelta(hours=1)).isoformat()
    )
...
def test_daemon_loop_exception():
    ...
    with patch.object(daemon, 'load_tasks', side_effect=Exception("boom")), \
         patch('code_puppy.scheduler.daemon.time.sleep', side_effect=fake_sleep):
        daemon._shutdown_requested = False
        daemon.run_scheduler_loop(check_interval=1)
...
def test_write_pid_file_failure(tmp_path):
    ...
    with patch.object(daemon, 'SCHEDULER_PID_FILE', '/nonexistent/dir/test.pid'), \
...
def test_start_daemon_background_unix():
    ...
    with patch.object(daemon, 'get_daemon_pid', side_effect=[None, 99999]), \
