...
def test_safe_rmtree_failure(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader.shutil.rmtree", side_effect=OSError("no")):
        d = tmp_path / "dir"
        d.mkdir()
...
def _make_mock_client(chunks=None, status_error=False, connect_error=False, generic_error=False):
    ...
    if status_error:
        ...
        mock_response.raise_for_status.side_effect = httpx.HTTPStatusError("err", request=resp.request, response=resp)
    elif connect_error:
...
def test_download_to_file_success(tmp_path):
    ...
    with patch("code_puppy.plugins.agent_skills.downloader.httpx.Client", return_value=mc):
...
def test_safe_extract_zip_outside_dir(tmp_path):
    ...
    extract_dir = tmp_path / "out"
    with zipfile.ZipFile(BytesIO(data)) as zf:
        with patch("code_puppy.plugins.agent_skills.downloader._is_within_directory", return_value=False):
...
def test_safe_extract_zip_exception(tmp_path):
    ...
    extract_dir = tmp_path / "out"
    with zipfile.ZipFile(BytesIO(data)) as zf:
        with patch("code_puppy.plugins.agent_skills.downloader._is_within_directory", side_effect=RuntimeError):
            assert _safe_extract_zip(zf, extract_dir) is False
...
def test_stage_normalized_install_exception(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader.shutil.copytree", side_effect=OSError):
        src = tmp_path / "src"
        src.mkdir()
        staging = tmp_path / "staging"
        staging.mkdir()
...
def test_already_installed_no_force(tmp_path):
    ...
    skill_dir.mkdir()
    r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path, force=False)
    assert not r.success and "already installed" in r.message

def test_force_reinstall_rmtree_fails(tmp_path):
    skill_dir = tmp_path / "myskill"
    skill_dir.mkdir()
    with patch("code_puppy.plugins.agent_skills.downloader._safe_rmtree", return_value=False):
        r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path, force=True)
    assert not r.success and "Failed to remove" in r.message

def test_download_failure(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=False):
...
def test_bad_zip_file(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=True):
        with patch("code_puppy.plugins.agent_skills.downloader.zipfile.ZipFile", side_effect=zipfile.BadZipFile):
            r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
    assert not r.success and "not a valid zip" in r.message

def test_zip_open_generic_exception(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=True):
        with patch("code_puppy.plugins.agent_skills.downloader.zipfile.ZipFile", side_effect=RuntimeError("boom")):
            r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
...
def test_unsafe_zip(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=True):
        ...
        with patch("code_puppy.plugins.agent_skills.downloader.zipfile.ZipFile", return_value=mock_zf):
            with patch("code_puppy.plugins.agent_skills.downloader._validate_zip_safety", return_value="too big"):
                r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
...
def test_extract_failure(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=True):
        ...
        mock_zf.__exit__ = MagicMock(return_value=False)
        with patch("code_puppy.plugins.agent_skills.downloader.zipfile.ZipFile", return_value=mock_zf):
            with patch("code_puppy.plugins.agent_skills.downloader._validate_zip_safety", return_value=None):
                with patch("code_puppy.plugins.agent_skills.downloader._safe_extract_zip", return_value=False):
                    r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
...
def test_no_extracted_root(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=True):
        ...
        with patch("code_puppy.plugins.agent_skills.downloader.zipfile.ZipFile", return_value=mock_zf):
            with patch("code_puppy.plugins.agent_skills.downloader._validate_zip_safety", return_value=None):
                with patch("code_puppy.plugins.agent_skills.downloader._safe_extract_zip", return_value=True):
                    with patch("code_puppy.plugins.agent_skills.downloader._determine_extracted_root", return_value=None):
                        r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
...
def test_stage_failure(tmp_path):
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=True):
        ...
        with patch("code_puppy.plugins.agent_skills.downloader.zipfile.ZipFile", return_value=mock_zf):
            with patch("code_puppy.plugins.agent_skills.downloader._validate_zip_safety", return_value=None):
                with patch("code_puppy.plugins.agent_skills.downloader._safe_extract_zip", return_value=True):
                    with patch("code_puppy.plugins.agent_skills.downloader._determine_extracted_root", return_value=Path("/fake")):
                        with patch("code_puppy.plugins.agent_skills.downloader._stage_normalized_install", return_value=None):
                            r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
    ...
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", return_value=True):
        ...
        with patch("code_puppy.plugins.agent_skills.downloader.zipfile.ZipFile", return_value=mock_zf):
            with patch("code_puppy.plugins.agent_skills.downloader._validate_zip_safety", return_value=None):
                with patch("code_puppy.plugins.agent_skills.downloader._safe_extract_zip", return_value=True):
                    with patch("code_puppy.plugins.agent_skills.downloader._determine_extracted_root", return_value=Path("/fake")):
                        with patch("code_puppy.plugins.agent_skills.downloader._stage_normalized_install", return_value=staged):
                            with patch("code_puppy.plugins.agent_skills.downloader.shutil.move", side_effect=OSError):
                                r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
...
def test_full_success(tmp_path):
    ...
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", side_effect=fake_download):
...
def test_full_success_cache_refresh_fails(tmp_path):
    ...
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", side_effect=fake_download):
        with patch("code_puppy.plugins.agent_skills.downloader.refresh_skill_cache", side_effect=RuntimeError("oops")):
            r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
    assert r.success
...
def test_post_install_missing_skill_md(tmp_path):
    ...
    with patch("code_puppy.plugins.agent_skills.downloader._download_to_file", side_effect=fake_download):
        with patch("code_puppy.plugins.agent_skills.downloader.shutil.move", side_effect=fake_move):
            r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
...
def test_full_success_nested_zip(tmp_path):
    zip_data = _make_zip_bytes({"myskill-main/SKILL.md": b"# Nested", "myskill-main/code.py": b"x=1"})
