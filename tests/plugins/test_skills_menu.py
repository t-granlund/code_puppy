...
def _make_metadata(name="skill-a", desc="A skill", tags=None, path="/tmp/skills/skill-a"):
    return SkillMetadata(
        name=name, description=desc, path=Path(path),
...
class TestSkillsMenuRendering:
    ...
    def _make_menu(self, skills=None, disabled=None, enabled=True):
        with patch(f"{_MOD}.get_skills_enabled", return_value=enabled), \
             patch(f"{_MOD}.get_skill_directories", return_value=[]), \
    ...
    @patch(f"{_MOD}.get_skill_resources", return_value=[])
    @patch(f"{_MOD}.parse_skill_metadata", return_value=_make_metadata(desc=None, tags=[]))
...
class TestSkillsMenuGetMetadata:
    def _make_menu(self):
        with patch(f"{_MOD}.get_skills_enabled", return_value=True), \
             patch(f"{_MOD}.get_skill_directories", return_value=[]), \
             patch(f"{_MOD}.get_disabled_skills", return_value=[]), \
             patch(f"{_MOD}.discover_skills", return_value=[_make_skill()]):
            from code_puppy.plugins.agent_skills.skills_menu import SkillsMenu
...
class TestSkillsMenuToggle:
    ...
    @patch(f"{_MOD}.discover_skills", return_value=[_make_skill()])
    @patch(f"{_MOD}.parse_skill_metadata", return_value=_make_metadata())
    def test_toggle_enables(self, mock_meta, mock_disc, mock_dis, mock_dirs, mock_en, mock_set, mock_refresh):
        from code_puppy.plugins.agent_skills.skills_menu import SkillsMenu
        menu = SkillsMenu()
    ...
    @patch(f"{_MOD}.discover_skills", return_value=[_make_skill()])
    @patch(f"{_MOD}.parse_skill_metadata", return_value=_make_metadata())
    def test_toggle_disables(self, mock_meta, mock_disc, mock_dis, mock_dirs, mock_en, mock_set, mock_refresh):
        from code_puppy.plugins.agent_skills.skills_menu import SkillsMenu
        menu = SkillsMenu()
        menu._toggle_current_skill()
    ...
    @patch(f"{_MOD}.discover_skills", return_value=[_make_skill()])
    @patch(f"{_MOD}.parse_skill_metadata", return_value=None)
    def test_toggle_no_metadata(self, mock_meta, mock_disc, mock_dis, mock_dirs, mock_en, mock_set, mock_refresh):
        from code_puppy.plugins.agent_skills.skills_menu import SkillsMenu
        menu = SkillsMenu()
        menu._toggle_current_skill()
        mock_set.assert_called_once_with("skill-a", True)
...
class TestSkillsMenuKeyBindings:
    ...
    def _run_with_key_capture(self, skills=None, disabled=None, callback=None):
        captured_kb = [None]
        orig_kb = __import__('prompt_toolkit.key_binding', fromlist=['KeyBindings']).KeyBindings
...
class TestPromptForDirectory:
    def test_prompt_returns_path(self):
        with patch("code_puppy.tools.common.safe_input", create=True, return_value="~/my-skills"):
    ...
    def test_prompt_keyboard_interrupt(self):
        with patch("code_puppy.tools.common.safe_input", create=True, side_effect=KeyboardInterrupt):
            from code_puppy.plugins.agent_skills.skills_menu import (
                _prompt_for_directory,
            )
            result = _prompt_for_directory()
    ...
    def test_prompt_eof_error(self):
        with patch("code_puppy.tools.common.safe_input", create=True, side_effect=EOFError):
            from code_puppy.plugins.agent_skills.skills_menu import (
                _prompt_for_directory,
            )
            result = _prompt_for_directory()
...
class TestShowDirectoriesMenu:
    ...
    def test_remove_dir(self, mock_dirs, mock_remove):
        with patch("code_puppy.tools.common.safe_input", create=True, side_effect=["1", "y"]):
            from code_puppy.plugins.agent_skills.skills_menu import (
                _show_directories_menu,
            )
            result = _show_directories_menu()
    ...
    def test_remove_dir_cancel(self, mock_dirs):
        with patch("code_puppy.tools.common.safe_input", create=True, side_effect=["1", "n"]):
            from code_puppy.plugins.agent_skills.skills_menu import (
                _show_directories_menu,
            )
            result = _show_directories_menu()
    ...
    def test_invalid_choice(self, mock_dirs):
        with patch("code_puppy.tools.common.safe_input", create=True, return_value="abc"):
            from code_puppy.plugins.agent_skills.skills_menu import (
                _show_directories_menu,
            )
            result = _show_directories_menu()
    ...
    def test_out_of_range(self, mock_dirs):
        with patch("code_puppy.tools.common.safe_input", create=True, return_value="99"):
            from code_puppy.plugins.agent_skills.skills_menu import (
                _show_directories_menu,
            )
            result = _show_directories_menu()
...
class TestShowSkillsMenu:
    ...
    def test_install_flow(self, mock_cls):
        ...
        mock_cls.return_value = mock_menu
        with patch("code_puppy.plugins.agent_skills.skills_install_menu.run_skills_install_menu", return_value=True):
            from code_puppy.plugins.agent_skills.skills_menu import show_skills_menu
            result = show_skills_menu()
            assert result is True
    ...
    def test_install_no_change(self, mock_cls):
        ...
        mock_menu.run.side_effect = ["install", "quit"]
        mock_cls.return_value = mock_menu
        with patch("code_puppy.plugins.agent_skills.skills_install_menu.run_skills_install_menu", return_value=False):
            from code_puppy.plugins.agent_skills.skills_menu import show_skills_menu
            result = show_skills_menu()
            assert result is False
...
class TestEnableDisableSkill:
    ...
    @patch(f"{_MOD}.parse_skill_metadata", return_value=_make_metadata(name="meta-name"))
