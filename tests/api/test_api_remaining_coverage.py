...
async def test_shutdown_pid_removal_error():
    ...
    with patch('code_puppy.api.app.get_pty_manager', return_value=mock_pty_manager, create=True), \
...
def test_unix_reader_loop_with_output():
    ...
    manager = PTYManager()
    output_data = []
    session = PTYSession(session_id="out", master_fd=999, pid=999, on_output=lambda d: output_data.append(d))
    session._running = True
