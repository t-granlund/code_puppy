...
class TestGetLexerForExtension:
    ...
    def test_get_lexer_by_name_exception(self):
        ...
        with patch("code_puppy.tools.common.get_lexer_by_name", side_effect=Exception("no lexer")):
...
class TestFormatDiffWithSyntaxHighlighting:
    ...
    def test_no_pygments(self):
        ...
        try:
            mod.PYGMENTS_AVAILABLE = False
            result = mod._format_diff_with_syntax_highlighting("diff text", "#002200", "#220000")
            assert isinstance(result, Text)
        finally:
            mod.PYGMENTS_AVAILABLE = orig
        ...
        with patch("code_puppy.tools.common.Application") as MockApp:
            ...
            with pytest.raises(KeyboardInterrupt):
                await arrow_select_async(
                    "Pick:", ["a", "b"],
                    preview_callback=lambda i: f"Preview {i}"
                )
...
class TestGetUserApproval:
    ...
    def test_reject_with_feedback(self):
        ...
        with patch("code_puppy.tools.common.arrow_select", return_value="💬 Reject with feedback (tell Biscuit what to change)"):
    ...
    def test_keyboard_interrupt(self):
        ...
        with patch("code_puppy.tools.common.arrow_select", side_effect=KeyboardInterrupt):
            with patch("code_puppy.tools.common.Console"):
                with patch("code_puppy.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_puppy.tools.common.emit_info"):
                        with patch("code_puppy.tools.common.emit_error"):
        ...
        with patch("code_puppy.tools.common.arrow_select", return_value="✓ Approve"):
            with patch("code_puppy.tools.common.Console"):
                with patch("code_puppy.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_puppy.tools.common.emit_info"):
                        with patch("code_puppy.tools.common.emit_success"):
                            with patch("code_puppy.tools.common.format_diff_with_colors", return_value=Text("diff")):
...
class TestGetUserApprovalAsync:
    ...
    async def test_approve(self):
        ...
        with patch("code_puppy.tools.common.arrow_select_async", new_callable=AsyncMock, return_value="✓ Approve"):
            with patch("code_puppy.tools.common.Console"):
                with patch("code_puppy.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_puppy.tools.common.emit_info"):
                        with patch("code_puppy.tools.common.emit_success"):
    ...
    async def test_reject(self):
        ...
        with patch("code_puppy.tools.common.arrow_select_async", new_callable=AsyncMock, return_value="✗ Reject"):
            with patch("code_puppy.tools.common.Console"):
                with patch("code_puppy.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_puppy.tools.common.emit_info"):
                        with patch("code_puppy.tools.common.emit_error"):
    ...
    async def test_reject_with_feedback(self):
        ...
        with patch("code_puppy.tools.common.arrow_select_async", new_callable=AsyncMock, return_value="💬 feedback"):
            with patch("code_puppy.tools.common.Prompt") as MockPrompt:
    ...
    async def test_keyboard_interrupt(self):
        ...
        with patch("code_puppy.tools.common.arrow_select_async", new_callable=AsyncMock, side_effect=KeyboardInterrupt):
            with patch("code_puppy.tools.common.Console"):
                with patch("code_puppy.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_puppy.tools.common.emit_info"):
                        with patch("code_puppy.tools.common.emit_error"):
