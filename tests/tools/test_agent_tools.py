...
class TestSaveSessionHistory:
    def test_save_new_session(self, tmp_path):
        ...
        with patch("code_puppy.tools.agent_tools._get_subagent_sessions_dir", return_value=tmp_path):
...
class TestRegisterListAgents:
    def test_register_and_call(self):
        ...
        ctx = MagicMock()
        with patch("code_puppy.tools.agent_tools.generate_group_id", return_value="grp"), \
             patch("code_puppy.tools.agent_tools.emit_info"), \
             patch("code_puppy.agents.get_available_agents", return_value={"a": "Agent A"}), \
             patch("code_puppy.agents.get_agent_descriptions", return_value={"a": "desc"}), \
    ...
    def test_register_error(self):
        ...
        with patch("code_puppy.tools.agent_tools.generate_group_id", return_value="grp"), \
             ...
             patch("code_puppy.config.get_banner_color", return_value="blue"), \
             patch("code_puppy.agents.get_available_agents", side_effect=Exception("boom")):
            result = captured["fn"](ctx)
...
class TestRegisterInvokeAgent:
    ...
    async def test_invalid_session_id(self):
        ...
        with patch("code_puppy.tools.agent_tools.generate_group_id", return_value="grp"), \
             patch("code_puppy.tools.agent_tools.emit_error"):
            result = await captured["fn"](ctx, agent_name="test", prompt="hi", session_id="INVALID_ID")
        assert result.error is not None
    ...
    async def test_agent_not_found(self):
        ...
        with patch("code_puppy.tools.agent_tools.generate_group_id", return_value="grp"), \
             ...
             patch("code_puppy.tools.agent_tools.get_session_context", return_value=None), \
             patch("code_puppy.tools.agent_tools.set_session_context"), \
             patch("code_puppy.tools.browser.terminal_tools.set_terminal_session", return_value=term_token), \
             patch("code_puppy.tools.browser.browser_manager.set_browser_session", return_value=browser_token), \
             patch("code_puppy.tools.browser.terminal_tools._terminal_session_var", fake_term_var), \
             patch("code_puppy.tools.browser.browser_manager._browser_session_var", fake_browser_var), \
             patch("code_puppy.agents.load_agent", side_effect=Exception("Agent not found")):
