...
+    print("new")
     return True
"""
        result = _format_diff_with_syntax_highlighting(diff, addition_color="#003300", deletion_color="#330000")
        assert isinstance(result, Text)

    def test_empty_diff(self):
        from code_puppy.tools.common import _format_diff_with_syntax_highlighting
        result = _format_diff_with_syntax_highlighting("", addition_color="#003300", deletion_color="#330000")
        assert isinstance(result, Text)

    def test_custom_colors(self):
        from code_puppy.tools.common import _format_diff_with_syntax_highlighting
        diff = "--- a/x.py\n+++ b/x.py\n@@ -1 +1 @@\n-old\n+new\n context"
        result = _format_diff_with_syntax_highlighting(diff, addition_color="#00ff00", deletion_color="#ff0000")
        assert isinstance(result, Text)
...
class TestGetUserApproval:
    ...
    @patch("time.sleep")
    def test_approved(self, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    def test_rejected(self, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval
        approved, feedback = get_user_approval("Test", "content", puppy_name="Rex")
        assert approved is False

    @patch("code_puppy.tools.common.arrow_select", return_value="\U0001f4ac Reject with feedback (tell Rex what to change)")
    @patch("code_puppy.tools.common.Prompt")
    @patch("code_puppy.tools.common.Console")
    @patch("code_puppy.tools.common.emit_info")
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    def test_rejected_with_feedback(self, mock_sleep, mock_await, mock_emit, MockConsole, MockPrompt, mock_select):
        from code_puppy.tools.common import get_user_approval
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    def test_keyboard_interrupt(self, mock_sleep, mock_await, mock_err, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval
        approved, feedback = get_user_approval("Test", "content", puppy_name="Rex")
        assert approved is False
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    def test_with_preview(self, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval
        approved, _ = get_user_approval("Test", "content", preview="--- a/x\n+++ b/x\n@@ -1 +1 @@\n-old\n+new", puppy_name="Rex")
        assert approved is True
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    def test_with_text_content(self, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval
    ...
    def test_default_puppy_name(self, mock_name, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval
...
class TestGetUserApprovalAsync:
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    async def test_approved(self, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval_async
        approved, feedback = await get_user_approval_async("Test", "content", puppy_name="Rex")
        assert approved is True
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    async def test_rejected(self, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval_async
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    async def test_rejected_feedback(self, mock_sleep, mock_await, mock_emit, MockConsole, MockPrompt, mock_select):
        from code_puppy.tools.common import get_user_approval_async
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    async def test_interrupt(self, mock_sleep, mock_await, mock_err, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval_async
        approved, _ = await get_user_approval_async("Test", "content", puppy_name="Rex")
        assert approved is False
    ...
    @patch("code_puppy.tools.command_runner.set_awaiting_user_input")
    @patch("time.sleep")
    async def test_with_preview(self, mock_sleep, mock_await, mock_emit, MockConsole, mock_select):
        from code_puppy.tools.common import get_user_approval_async
        approved, _ = await get_user_approval_async("Test", "content", preview="--- a/x\n+++ b/x\n@@ -1 +1 @@\n-old\n+new", puppy_name="Rex")
        assert approved is True
