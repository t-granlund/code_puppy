...
class TestShellCommandKeyboardContext:
    def test_context_manager(self):
        ...
        with patch("code_puppy.tools.command_runner._spawn_ctrl_x_key_listener", return_value=None):
...
class TestRunShellCommand:
    ...
    async def test_empty_command(self):
        ...
        with patch("code_puppy.callbacks.on_run_shell_command", new_callable=AsyncMock, return_value=[]):
            with patch("code_puppy.tools.command_runner.emit_error"):
    ...
    async def test_blocked_by_callback(self):
        ...
        ctx = MagicMock(spec=RunContext)
        blocked_result = {"blocked": True, "error_message": "nope", "reasoning": "risky"}
        with patch("code_puppy.callbacks.on_run_shell_command", new_callable=AsyncMock, return_value=[blocked_result]):
    ...
    async def test_yolo_mode_executes(self):
        ...
        with patch("code_puppy.callbacks.on_run_shell_command", new_callable=AsyncMock, return_value=[]):
            with patch("code_puppy.config.get_yolo_mode", return_value=True):
                with patch("code_puppy.tools.command_runner.is_subagent", return_value=False):
                    with patch("code_puppy.tools.command_runner._execute_shell_command", new_callable=AsyncMock, return_value=mock_output):
    ...
    async def test_subagent_runs_silently(self):
        ...
        with patch("code_puppy.callbacks.on_run_shell_command", new_callable=AsyncMock, return_value=[]):
            with patch("code_puppy.config.get_yolo_mode", return_value=False):
                with patch("code_puppy.tools.command_runner.is_subagent", return_value=True):
                    with patch("code_puppy.tools.command_runner._execute_shell_command", new_callable=AsyncMock, return_value=mock_output):
                        result = await run_shell_command(ctx, "echo hi", timeout=10)
        assert result.success is True
            ...
            with patch("code_puppy.config.get_yolo_mode", return_value=False):
                with patch("code_puppy.tools.command_runner.is_subagent", return_value=False):
                    with patch("sys.stdin") as mock_stdin:
                        ...
                        with patch("code_puppy.tools.command_runner.get_user_approval_async", new_callable=AsyncMock, return_value=(False, None)):
                            with patch("code_puppy.config.get_puppy_name", return_value="buddy"):
                                result = await run_shell_command(ctx, "echo hi", timeout=10)
        ...
        with patch("code_puppy.callbacks.on_run_shell_command", new_callable=AsyncMock, return_value=[]):
            with patch("code_puppy.config.get_yolo_mode", return_value=False):
                with patch("code_puppy.tools.command_runner.is_subagent", return_value=False):
                    with patch("sys.stdin") as mock_stdin:
                        mock_stdin.isatty.return_value = True
                        with patch("code_puppy.tools.command_runner.get_user_approval_async", new_callable=AsyncMock, return_value=(False, "use ls instead")):
                            with patch("code_puppy.config.get_puppy_name", return_value="buddy"):
                                result = await run_shell_command(ctx, "echo hi", timeout=10)
...
class TestExecuteShellCommand:
    ...
    async def test_executes(self):
        ...
        mock_result = ShellCommandOutput(
            success=True, command="echo hi", stdout="hi", stderr="",
        ...
        with patch("code_puppy.tools.command_runner.get_message_bus") as mock_bus:
            ...
            with patch("code_puppy.messaging.spinner.pause_all_spinners"):
                with patch("code_puppy.messaging.spinner.resume_all_spinners"):
                    with patch("code_puppy.tools.command_runner._acquire_keyboard_context"):
                        with patch("code_puppy.tools.command_runner._release_keyboard_context"):
                            with patch("code_puppy.tools.command_runner._run_command_inner", new_callable=AsyncMock, return_value=mock_result):
                                result = await _execute_shell_command("echo hi", None, 10, "grp")
...
class TestRunCommandInner:
    ...
    async def test_exception(self):
        ...
        with patch("code_puppy.tools.command_runner._run_command_sync", side_effect=Exception("boom")):
            with patch("code_puppy.tools.command_runner.emit_error"):
                try:
...
class TestSignalHandlers:
    def test_handle_ctrl_x_press(self):
        ...
        with patch("code_puppy.tools.command_runner.emit_warning"):
            with patch("code_puppy.tools.command_runner.kill_all_running_shell_processes", return_value=0):
