...
class TestHelpers:
    ...
    def test_emit_uc_message(self):
        ...
        with patch("code_puppy.tools.universal_constructor.get_message_bus") as mock_bus:
    ...
    def test_build_summary_list(self):
        ...
        r = UniversalConstructorOutput(action="list", success=True,
    ...
    def test_build_summary_call(self):
        ...
        r = UniversalConstructorOutput(action="call", success=True,
            call_result=UCCallOutput(success=True, tool_name="t", result="x", execution_time=1.5))
    ...
    def test_build_summary_create(self):
        ...
        r = UniversalConstructorOutput(action="create", success=True,
            create_result=UCCreateOutput(success=True, tool_name="t", source_path="/x", preview="p"))
    ...
    def test_build_summary_update(self):
        ...
        r = UniversalConstructorOutput(action="update", success=True,
            update_result=UCUpdateOutput(success=True, tool_name="t", source_path="/x", preview="p"))
    ...
    def test_build_summary_info(self):
        ...
        tool = UCToolInfo(meta=meta, signature="", source_path="", function_name="f", docstring="")
        r = UniversalConstructorOutput(action="info", success=True,
...
class TestUniversalConstructorImpl:
    ...
    async def test_routes_create(self, mock_create, mock_emit):
        ...
        mock_create.return_value = UniversalConstructorOutput(action="create", success=True)
        ctx = MagicMock()
    ...
    async def test_routes_update(self, mock_update, mock_emit):
        ...
        mock_update.return_value = UniversalConstructorOutput(action="update", success=True)
        ctx = MagicMock()
...
class TestHandleListAction:
    def test_success(self):
        ...
        with patch("code_puppy.plugins.universal_constructor.registry.get_registry", return_value=mock_reg):
    ...
    def test_error(self):
        from code_puppy.tools.universal_constructor import _handle_list_action
        with patch("code_puppy.plugins.universal_constructor.registry.get_registry", side_effect=Exception("boom")):
            r = _handle_list_action(MagicMock())
        assert not r.success
...
class TestHandleCallAction:
    ...
    def test_timeout(self):
        ...
        tool.source_path = None
        mock_reg.get_tool.return_value = tool
        def slow_func(): time.sleep(100)
        mock_reg.get_tool_function.return_value = slow_func
        with patch("code_puppy.plugins.universal_constructor.registry.get_registry", return_value=mock_reg):
            with patch("code_puppy.tools.universal_constructor.ThreadPoolExecutor") as mock_exec:
                from concurrent.futures import TimeoutError as FTE
                mock_future = MagicMock()
                mock_future.result.side_effect = FTE()
                mock_exec.return_value.__enter__ = MagicMock(return_value=MagicMock(submit=MagicMock(return_value=mock_future)))
    ...
    def test_type_error(self):
        ...
        tool.source_path = None
        mock_reg.get_tool.return_value = tool
        def bad_func(): raise TypeError("wrong args")
...
class TestHandleCreateAction:
    ...
    def test_syntax_error(self):
        from code_puppy.tools.universal_constructor import _handle_create_action
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val:
    ...
    def test_no_functions(self):
        from code_puppy.tools.universal_constructor import _handle_create_action
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             patch("code_puppy.plugins.universal_constructor.sandbox.extract_function_info") as mock_ext:
    ...
    def test_no_name(self):
        ...
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             patch("code_puppy.plugins.universal_constructor.sandbox.extract_function_info") as mock_ext, \
             patch("code_puppy.plugins.universal_constructor.sandbox._extract_tool_meta", return_value=None):
            mock_val.return_value = MagicMock(valid=True)
    ...
    def test_meta_validation_error(self):
        ...
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             patch("code_puppy.plugins.universal_constructor.sandbox.extract_function_info") as mock_ext, \
             patch("code_puppy.plugins.universal_constructor.sandbox._extract_tool_meta", return_value={"name": "t"}), \
             patch("code_puppy.plugins.universal_constructor.sandbox._validate_tool_meta", return_value=["missing field"]):
            mock_val.return_value = MagicMock(valid=True)
    ...
    def test_write_error(self):
        ...
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             patch("code_puppy.plugins.universal_constructor.sandbox.extract_function_info") as mock_ext, \
             patch("code_puppy.plugins.universal_constructor.sandbox._extract_tool_meta", return_value=None), \
             patch("code_puppy.plugins.universal_constructor.sandbox.check_dangerous_patterns") as mock_safe, \
    ...
    def test_success_with_namespace(self, tmp_path):
        ...
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             ...
             patch("code_puppy.tools.universal_constructor._run_ruff_format", return_value=None), \
             patch("code_puppy.plugins.universal_constructor.registry.get_registry"):
            mock_val.return_value = MagicMock(valid=True)
            mock_ext.return_value = MagicMock(functions=[func], warnings=[])
            mock_safe.return_value = MagicMock(warnings=[])
            r = _handle_create_action(MagicMock(), "api.weather", "def f(): pass", "desc")
        assert r.success
    ...
    def test_success_with_existing_meta(self, tmp_path):
        ...
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             patch("code_puppy.plugins.universal_constructor.sandbox.extract_function_info") as mock_ext, \
             patch("code_puppy.plugins.universal_constructor.sandbox._extract_tool_meta", return_value=meta), \
             patch("code_puppy.plugins.universal_constructor.sandbox._validate_tool_meta", return_value=[]), \
             patch("code_puppy.plugins.universal_constructor.sandbox.check_dangerous_patterns") as mock_safe, \
             patch("code_puppy.plugins.universal_constructor.USER_UC_DIR", tmp_path), \
             patch("code_puppy.tools.universal_constructor._run_ruff_format", return_value="warn"), \
             patch("code_puppy.plugins.universal_constructor.registry.get_registry"):
            mock_val.return_value = MagicMock(valid=True)
    ...
    def test_registry_reload_fail(self, tmp_path):
        ...
        with patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             ...
             patch("code_puppy.plugins.universal_constructor.USER_UC_DIR", tmp_path), \
             patch("code_puppy.tools.universal_constructor._run_ruff_format", return_value=None), \
             patch("code_puppy.plugins.universal_constructor.registry.get_registry") as mock_reg:
            mock_val.return_value = MagicMock(valid=True)
            mock_ext.return_value = MagicMock(functions=[func], warnings=[])
            mock_safe.return_value = MagicMock(warnings=[])
...
class TestHandleUpdateAction:
    ...
    def test_syntax_error(self, tmp_path):
        ...
        mock_reg.get_tool.return_value = tool
        with patch("code_puppy.plugins.universal_constructor.registry.get_registry", return_value=mock_reg), \
             patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val:
    ...
    def test_invalid_meta(self, tmp_path):
        ...
        with patch("code_puppy.plugins.universal_constructor.registry.get_registry", return_value=mock_reg), \
             patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax") as mock_val, \
             patch("code_puppy.plugins.universal_constructor.sandbox._extract_tool_meta", return_value={"name": "t"}), \
             patch("code_puppy.plugins.universal_constructor.sandbox._validate_tool_meta", return_value=["err"]):
            mock_val.return_value = MagicMock(valid=True)
            r = _handle_update_action(MagicMock(), "t", "def f(): pass", None)
        assert not r.success
    ...
    def test_update_exception(self, tmp_path):
        ...
        mock_reg.get_tool.return_value = tool
        with patch("code_puppy.plugins.universal_constructor.registry.get_registry", return_value=mock_reg), \
             patch("code_puppy.plugins.universal_constructor.sandbox.validate_syntax", side_effect=Exception("boom")):
            r = _handle_update_action(MagicMock(), "t", "code", None)
        assert not r.success
...
class TestHandleUpdateFormatWarning:
    def test_format_warning(self, tmp_path):
        ...
        with patch("code_puppy.plugins.universal_constructor.registry.get_registry", return_value=mock_reg), \
             ...
             patch("code_puppy.plugins.universal_constructor.sandbox._extract_tool_meta", return_value={"name": "t"}), \
             patch("code_puppy.plugins.universal_constructor.sandbox._validate_tool_meta", return_value=[]), \
             patch("code_puppy.tools.universal_constructor._run_ruff_format", return_value="ruff warning"):
            mock_val.return_value = MagicMock(valid=True)
            r = _handle_update_action(MagicMock(), "t", "def f(): pass", None)
        assert r.success
...
class TestImplRegistration:
    ...
    async def test_registered_tool_calls_impl(self):
        ...
        ctx = MagicMock()
        with patch("code_puppy.tools.universal_constructor.universal_constructor_impl") as mock_impl:
            from code_puppy.tools.universal_constructor import (
                UniversalConstructorOutput,
            )
            mock_impl.return_value = UniversalConstructorOutput(action="list", success=True)
