...
def _mock_parse_result(prompt="hello", warnings=None, attachments=None, link_attachments=None):
    m = MagicMock()
    m.prompt = prompt
...
def _interactive_patches():
    return {
        "code_puppy.cli_runner.print_truecolor_warning": MagicMock(),
        "code_puppy.cli_runner.get_cancel_agent_display_name": MagicMock(return_value="Ctrl+C"),
        "code_puppy.cli_runner.reset_windows_terminal_ansi": MagicMock(),
        "code_puppy.cli_runner.reset_windows_terminal_full": MagicMock(),
        "code_puppy.cli_runner.save_command_to_history": MagicMock(),
        "code_puppy.cli_runner.finalize_autosave_session": MagicMock(return_value="session-1"),
        "code_puppy.cli_runner.COMMAND_HISTORY_FILE": "/tmp/test_history",
        "code_puppy.command_line.motd.print_motd": MagicMock(),
        "code_puppy.command_line.onboarding_wizard.should_show_onboarding": MagicMock(return_value=False),
        "code_puppy.config.auto_save_session_if_enabled": MagicMock(),
    }


async def _run_interactive(renderer, patches_dict, input_fn, agent=None, initial_command=None, extra_patches=None):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch(
            ...
            side_effect=input_fn if callable(input_fn) and not isinstance(input_fn, AsyncMock) else input_fn,
        ))
        stack.enter_context(patch(
...
class TestMain:
    ...
    async def _run_main(self, argv, extra_patches=None, base_overrides=None):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("code_puppy.messaging.SynchronousInteractiveRenderer", return_value=_mock_renderer()))
            stack.enter_context(patch("code_puppy.messaging.RichConsoleRenderer", return_value=_mock_renderer()))
            stack.enter_context(patch("code_puppy.messaging.get_global_queue", return_value=MagicMock()))
            stack.enter_context(patch("code_puppy.messaging.get_message_bus", return_value=MagicMock()))
    ...
    async def test_no_available_port(self):
        await self._run_main(
            ["code-puppy", "-p", "test"],
            base_overrides={
                "code_puppy.cli_runner.find_available_port": MagicMock(return_value=None),
            },
        )
    ...
    async def test_model_valid(self):
        ...
        await self._run_main(
            ...
            extra_patches={
                ...
                "code_puppy.config._validate_model_exists": MagicMock(return_value=True),
            },
        )
        ...
        with pytest.raises(SystemExit):
            await self._run_main(
                ...
                extra_patches={
                    ...
                    "code_puppy.config._validate_model_exists": MagicMock(return_value=False),
    ...
    async def test_model_validation_exception(self):
        with pytest.raises(SystemExit):
            await self._run_main(
                ...
                extra_patches={
                    "code_puppy.config.set_model_name": MagicMock(),
                    "code_puppy.config._validate_model_exists": MagicMock(side_effect=RuntimeError("boom")),
                },
            )
    ...
    async def test_agent_valid(self):
        ...
        await self._run_main(
            ...
            extra_patches={
                "code_puppy.cli_runner.execute_single_prompt": AsyncMock(),
                "code_puppy.agents.agent_manager.get_available_agents": MagicMock(return_value={"code-puppy": {}}),
    ...
    async def test_agent_exception(self):
        with pytest.raises(SystemExit):
            await self._run_main(
                ...
                extra_patches={
                    "code_puppy.agents.agent_manager.get_available_agents": MagicMock(side_effect=RuntimeError("boom")),
                },
            )
    ...
    async def test_version_check_with_callbacks(self):
        ...
        with ExitStack() as stack:
            stack.enter_context(patch.dict(os.environ, {"NO_VERSION_UPDATE": ""}, clear=False))
            stack.enter_context(patch("sys.argv", ["code-puppy", "-p", "hi"]))
            stack.enter_context(patch("code_puppy.messaging.SynchronousInteractiveRenderer", return_value=_mock_renderer()))
            stack.enter_context(patch("code_puppy.messaging.RichConsoleRenderer", return_value=_mock_renderer()))
            stack.enter_context(patch("code_puppy.messaging.get_global_queue", return_value=MagicMock()))
            stack.enter_context(patch("code_puppy.messaging.get_message_bus", return_value=MagicMock()))
            stack.enter_context(patch("code_puppy.cli_runner.execute_single_prompt", new_callable=AsyncMock))
            _apply_patches(stack, patches)
            from code_puppy.cli_runner import main
            await main()
...
class TestInteractiveMode:
    ...
    async def test_keyboard_interrupt_continues(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=False),
            },
        )
    ...
    async def test_keyboard_interrupt_stops_wiggum(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=True),
    ...
    async def test_clear_command(self):
        ...
        await _run_interactive(
            ...
            agent=agent,
            extra_patches={
                "code_puppy.cli_runner.get_current_agent": MagicMock(return_value=agent),
                "code_puppy.cli_runner.get_clipboard_manager": MagicMock(return_value=_mock_clipboard([b"img"])),
            },
        )
    ...
    async def test_slash_command_handled(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.command_line.command_handler.handle_command": MagicMock(return_value=True),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("/help")),
            },
        )
    ...
    async def test_slash_command_returns_prompt(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.command_line.command_handler.handle_command": MagicMock(return_value="run this"),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("/custom")),
                "code_puppy.cli_runner.run_prompt_with_attachments": AsyncMock(return_value=(mock_result, MagicMock())),
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=False),
            },
        )
    ...
    async def test_slash_command_exception(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.command_line.command_handler.handle_command": MagicMock(side_effect=RuntimeError("cmd error")),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("/bad")),
            },
        )
    ...
    async def test_normal_prompt_execution(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                "code_puppy.cli_runner.run_prompt_with_attachments": AsyncMock(return_value=(mock_result, MagicMock())),
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=False),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("write hello")),
            },
        )
    ...
    async def test_prompt_returns_none_cancelled(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.cli_runner.run_prompt_with_attachments": AsyncMock(return_value=(None, MagicMock())),
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=False),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("write hello")),
            },
        )
    ...
    async def test_prompt_exception(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.cli_runner.run_prompt_with_attachments": AsyncMock(side_effect=RuntimeError("agent error")),
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=False),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("write hello")),
                "code_puppy.messaging.queue_console.get_queue_console": MagicMock(return_value=MagicMock()),
            },
        )
    ...
    async def test_empty_input_skipped(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("   ")),
            },
        )
    ...
    async def test_initial_command_error(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                "code_puppy.cli_runner.get_current_agent": MagicMock(return_value=agent),
                "code_puppy.cli_runner.run_prompt_with_attachments": AsyncMock(side_effect=RuntimeError("fail")),
            },
        )
    ...
    async def test_autosave_load_non_tty(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.command_line.command_handler.handle_command": MagicMock(return_value="__AUTOSAVE_LOAD__"),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("/autosave_load")),
        ...
        with patch.dict(os.environ, {"CODE_PUPPY_NO_TUI": ""}, clear=False):
            await _run_interactive(
                ...
                extra_patches={
                    ...
                    "sys.stdin": mock_stdin,
                    "sys.stdout": mock_stdout,
                    "code_puppy.command_line.autosave_menu.interactive_autosave_picker": AsyncMock(return_value=None),
                },
            )
        ...
        with patch.dict(os.environ, {"CODE_PUPPY_NO_TUI": ""}, clear=False):
            await _run_interactive(
                ...
                extra_patches={
                    ...
                    "sys.stdin": mock_stdin,
                    "sys.stdout": mock_stdout,
                    "code_puppy.command_line.autosave_menu.interactive_autosave_picker": AsyncMock(return_value="my-session"),
                    "code_puppy.session_storage.load_session": MagicMock(return_value=[MagicMock()]),
    ...
    async def test_autosave_load_exception(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "sys.stdin": mock_stdin,
                "sys.stdout": mock_stdout,
                "code_puppy.session_storage.restore_autosave_interactively": AsyncMock(side_effect=RuntimeError("fail")),
            },
        )
    ...
    async def test_slash_command_returns_false(self):
        ...
        await _run_interactive(
            ...
            fake_input,
            extra_patches={
                "code_puppy.command_line.command_handler.handle_command": MagicMock(return_value=False),
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("/unknown")),
                "code_puppy.cli_runner.run_prompt_with_attachments": AsyncMock(return_value=(mock_result, MagicMock())),
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=False),
            },
        )
    ...
    async def test_wiggum_loop(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "code_puppy.command_line.wiggum_state.get_wiggum_prompt": MagicMock(return_value="repeat"),
                "code_puppy.command_line.wiggum_state.increment_wiggum_count": MagicMock(return_value=1),
    ...
    async def test_wiggum_no_prompt_stops(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("write hello")),
                "code_puppy.command_line.wiggum_state.is_wiggum_active": fake_wiggum,
                "code_puppy.command_line.wiggum_state.get_wiggum_prompt": MagicMock(return_value=None),
                "code_puppy.command_line.wiggum_state.stop_wiggum": mock_stop,
            },
        )
        mock_stop.assert_called()
    ...
    async def test_onboarding_chatgpt(self):
        ...
        patches["code_puppy.command_line.onboarding_wizard.should_show_onboarding"] = MagicMock(return_value=True)
        ...
        await _run_interactive(
            ...
            AsyncMock(return_value="/exit"),
            extra_patches={
                "concurrent.futures.ThreadPoolExecutor": MagicMock(return_value=mock_executor),
                "code_puppy.command_line.onboarding_wizard.run_onboarding_wizard": AsyncMock(return_value="chatgpt"),
    ...
    async def test_onboarding_exception(self):
        patches = _interactive_patches()
        patches["code_puppy.command_line.onboarding_wizard.should_show_onboarding"] = MagicMock(side_effect=RuntimeError("fail"))
    ...
    async def test_motd_exception(self):
        patches = _interactive_patches()
        patches["code_puppy.command_line.motd.print_motd"] = MagicMock(side_effect=RuntimeError("motd fail"))
    ...
    async def test_clear_no_clipboard_images(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                "code_puppy.cli_runner.get_current_agent": MagicMock(return_value=agent),
                "code_puppy.cli_runner.get_clipboard_manager": MagicMock(return_value=_mock_clipboard()),
            },
        )
...
class TestInteractiveModeEdgeCases:
    ...
    async def test_exit_with_running_task(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                "code_puppy.cli_runner.run_prompt_with_attachments": fake_run,
                "code_puppy.cli_runner.parse_prompt_attachments": MagicMock(return_value=_mock_parse_result("do work")),
                "code_puppy.command_line.wiggum_state.is_wiggum_active": MagicMock(return_value=False),
            },
        )
    ...
    async def test_clear_with_clipboard_images(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                "code_puppy.cli_runner.get_current_agent": MagicMock(return_value=agent),
                "code_puppy.command_line.clipboard.get_clipboard_manager": MagicMock(return_value=clip),
            },
        )
...
class TestMainUvxAndEdgeCases:
    ...
    async def test_uvx_alternate_cancel_key(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("code_puppy.terminal_utils.disable_windows_ctrl_c"))
            stack.enter_context(patch("code_puppy.terminal_utils.set_keep_ctrl_c_disabled"))
    ...
    async def test_initial_command_awaiting_input(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                "code_puppy.cli_runner.get_current_agent": MagicMock(return_value=agent),
                "code_puppy.cli_runner.run_prompt_with_attachments": AsyncMock(return_value=(mock_result, MagicMock())),
                "code_puppy.tools.command_runner.is_awaiting_user_input": MagicMock(return_value=True),
            },
        )
...
class TestImportErrorFallbacks:
    ...
    async def test_is_awaiting_user_input_import_error(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch(
                "code_puppy.command_line.prompt_toolkit_completion.get_input_with_combined_completion",
                new_callable=AsyncMock, return_value="/exit",
            ))
            stack.enter_context(patch(
                "code_puppy.command_line.prompt_toolkit_completion.get_prompt_with_active_model",
                return_value="> ",
            ...
            stack.enter_context(patch("code_puppy.cli_runner.get_current_agent", return_value=agent))
            stack.enter_context(patch(
                "code_puppy.cli_runner.run_prompt_with_attachments",
                new_callable=AsyncMock, return_value=(mock_result, MagicMock()),
            ))
...
class TestMainEntryAdditional:
    ...
    def test_keyboard_interrupt_stderr_output(self, mock_run):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch("code_puppy.cli_runner.get_use_dbos", return_value=False))
            result = main_entry()
            assert result == 0
