...
class TestExecuteNoArgs:
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "total_size_bytes": 2048, "rotated_count": 0, "line_count": 50, "exists": True,
    })
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "total_size_bytes": 1024 * 1024 * 2, "rotated_count": 3, "line_count": 1000, "exists": True,
    })
...
class TestExecuteClear:
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "exists": True, "rotated_count": 1, "line_count": 10, "total_size_bytes": 100,
    })
    @patch(f"{MODULE}.emit_info")
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "exists": False, "rotated_count": 0, "line_count": 0, "total_size_bytes": 0,
    })
    @patch(f"{MODULE}.emit_info")
...
class TestExecuteShowLogs:
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "line_count": 2, "exists": True, "total_size_bytes": 100, "rotated_count": 0,
    })
    @patch(f"{MODULE}.get_log_file_path", return_value="/tmp/test.log")
    @patch(f"{MODULE}.emit_info")
    def test_show_default_lines(self, mock_info, mock_path, mock_stats, mock_read, mock_find):
        cmd = _make_cmd()
        cmd.execute(["my-srv"])
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "line_count": 100, "exists": True, "total_size_bytes": 100, "rotated_count": 0,
    })
    @patch(f"{MODULE}.get_log_file_path", return_value="/tmp/test.log")
    @patch(f"{MODULE}.emit_info")
    def test_show_with_hint(self, mock_info, mock_path, mock_stats, mock_read, mock_find):
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "line_count": 0, "exists": True, "total_size_bytes": 0, "rotated_count": 0,
    })
    @patch(f"{MODULE}.get_log_file_path", return_value="/tmp/test.log")
    @patch(f"{MODULE}.emit_info")
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "exists": False, "line_count": 0, "total_size_bytes": 0, "rotated_count": 0,
    })
    ...
    @patch(f"{UTILS}.find_server_id_by_name", return_value=None)
    @patch(f"{MODULE}.get_log_stats", return_value={
        "exists": True, "line_count": 5, "total_size_bytes": 100, "rotated_count": 0,
    })
    @patch(f"{MODULE}.read_logs", return_value=["old log line"])
    @patch(f"{MODULE}.get_log_file_path", return_value="/tmp/old.log")
    @patch(f"{MODULE}.emit_info")
    def test_server_not_configured_but_has_logs(self, mock_info, mock_path, mock_read, mock_stats, mock_find):
        cmd = _make_cmd()
        cmd.execute(["old-srv"])
...
class TestExecuteLinesParsing:
    ...
    @patch(f"{MODULE}.get_log_stats", return_value={
        "line_count": 1, "exists": True, "total_size_bytes": 10, "rotated_count": 0,
    })
    @patch(f"{MODULE}.get_log_file_path", return_value="/tmp/t.log")
    @patch(f"{MODULE}.emit_info")
    def test_invalid_number(self, mock_info, mock_path, mock_stats, mock_read, mock_find):
        cmd = _make_cmd()
    ...
    @patch(f"{MODULE}.get_log_file_path", return_value="/tmp/t.log")
    @patch(f"{MODULE}.emit_info")
    def test_negative_number(self, mock_info, mock_path, mock_stats, mock_read, mock_find):
        cmd = _make_cmd()
        cmd.execute(["srv", "-5"])
        # Should fall back to 50
        mock_read.assert_called_once_with("srv", lines=50)
    ...
    @patch(f"{MODULE}.get_log_file_path", return_value="/tmp/t.log")
    @patch(f"{MODULE}.emit_info")
    def test_custom_line_count(self, mock_info, mock_path, mock_stats, mock_read, mock_find):
        cmd = _make_cmd()
        cmd.execute(["srv", "100"])
