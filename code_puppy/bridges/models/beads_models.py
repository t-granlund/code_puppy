...
class Gate(BaseModel):
    ...
    blocking_issues: List[str] = Field(
        default_factory=list, description="Issues blocking this gate"
    )

    # Relationships
    bead_id: Optional[str] = Field(default=None, description="Associated bead ID")
    molecule_id: Optional[str] = Field(
        default=None, description="Associated molecule ID"
    )
...
class Formula(BaseModel):
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Formula dependencies"
    )
