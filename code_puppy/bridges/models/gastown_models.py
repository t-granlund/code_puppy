...
class Polecat(BaseModel):
    ...
    specialty: Optional[str] = Field(
        default=None, description="Agent specialty (e.g., 'rust', 'frontend')"
    )
    runtime: str = Field(
        default="claude", description="AI runtime (claude, codex, cursor, etc.)"
    )
    ...
    hook_path: Optional[str] = Field(
        default=None, description="Path to git-backed hook"
    )
    ...
    current_session_id: Optional[str] = Field(
        default=None, description="Current session ID if active"
    )
...
class Convoy(BaseModel):
    ...
    # Work content
    bead_ids: List[str] = Field(
        default_factory=list, description="Bead/issue IDs in convoy"
    )
    completed_beads: List[str] = Field(
        default_factory=list, description="Completed bead IDs"
    )
    ...
    require_human_review: bool = Field(
        default=False, description="Require human review"
    )
...
class Rig(BaseModel):
    ...
    is_mountain_capable: bool = Field(
        default=False, description="Can run mountain mode"
    )
    ...
    agent_ids: List[str] = Field(
        default_factory=list, description="Registered agent IDs"
    )
    polecat_count: int = Field(default=0, description="Number of polecats")

    # Convoys
    active_convoy_ids: List[str] = Field(
        default_factory=list, description="Active convoy IDs"
    )
    archived_convoy_ids: List[str] = Field(
        default_factory=list, description="Archived convoy IDs"
    )
...
class Mail(BaseModel):
    ...
    sent_at: datetime = Field(
        default_factory=datetime.now, description="Send timestamp"
    )
...
class Escalation(BaseModel):
    ...
    escalation_type: str = Field(
        description="Type of escalation (stuck_agent, failed_task, etc.)"
    )
    severity: str = Field(default="medium", description="Escalation severity")

    # Source
    source_agent_id: Optional[str] = Field(
        default=None, description="Agent that triggered escalation"
    )
    source_convoy_id: Optional[str] = Field(
        default=None, description="Convoy that triggered escalation"
    )
