...
class Commit(BaseModel):
    ...
    branch: Optional[str] = Field(default=None, description="Branch containing this commit")
...
class Branch(BaseModel):
    ...
    # Remote info
    remote: Optional[str] = Field(default=None, description="Remote name if remote branch")
    tracking_branch: Optional[str] = Field(default=None, description="Upstream tracking branch")
    ...
    last_commit_at: Optional[datetime] = Field(default=None, description="Last commit timestamp")
...
class Diff(BaseModel):
    ...
    columns_dropped: List[str] = Field(default_factory=list, description="Columns dropped")
    columns_modified: List[str] = Field(default_factory=list, description="Columns modified")
...
class Table(BaseModel):
    ...
    # State
    is_system_table: bool = Field(default=False, description="Whether this is a system table")
    is_temporary: bool = Field(default=False, description="Whether this is a temporary table")
...
class QueryResult(BaseModel):
    ...
    # For DML queries
    rows_affected: Optional[int] = Field(default=None, description="Rows affected (for INSERT/UPDATE/DELETE)")
...
class Schema(BaseModel):
    ...
    # Keys
    primary_key: List[str] = Field(default_factory=list, description="Primary key columns")
    unique_keys: List[List[str]] = Field(default_factory=list, description="Unique key column sets")
    ...
    # DDL
    create_statement: Optional[str] = Field(default=None, description="CREATE TABLE statement")
...
class MergeResult(BaseModel):
    ...
    fast_forward: bool = Field(default=False, description="Whether fast-forward was possible")
    
    # Conflicts
    has_conflicts: bool = Field(default=False, description="Whether there are conflicts")
    conflicted_tables: List[str] = Field(default_factory=list, description="Tables with conflicts")
    
    # Stats
    tables_added: List[str] = Field(default_factory=list, description="Tables added")
    tables_deleted: List[str] = Field(default_factory=list, description="Tables deleted")
    tables_modified: List[str] = Field(default_factory=list, description="Tables modified")
...
class RepositoryStatus(BaseModel):
    ...
    # Status
    status: DoltStatus = Field(default=DoltStatus.CLEAN, description="Repository status")
    
    # Working tree
    staged_tables: List[str] = Field(default_factory=list, description="Staged tables")
    unstaged_tables: List[str] = Field(default_factory=list, description="Unstaged modified tables")
    untracked_tables: List[str] = Field(default_factory=list, description="Untracked tables")
    
    # Merge state
    merge_in_progress: bool = Field(default=False, description="Whether merge is in progress")
    merge_source: Optional[str] = Field(default=None, description="Merge source if in progress")
