...
def _deep_merge_hooks(base: Dict[str, Any], overlay: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for event_type, hook_groups in overlay.items():
        ...
        if event_type not in merged:
            ...
            logger.debug(f"Merged {len(hook_groups)} hook group(s) for event '{event_type}'")
        else:
            # Type mismatch or unexpected structure, keep base
            logger.warning(f"Cannot merge event type '{event_type}': type mismatch or unexpected structure")
...
def load_hooks_config() -> Optional[Dict[str, Any]]:
    ...
    event_count = len([event for event in merged_config.keys() if not event.startswith('_')])
